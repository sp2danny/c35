...
C35::AnimReflection C35::Unit::GetIcon()
{
...
C35::Unit* C35::UnitType::MakeNewFromType(int ut_idx,Player*p)
{
	...
	for(int j=0;j<256;++j) u.buffer[j]=j;
	...
	case Orders::move:
		{
			...
			if( h.IsLand() && landunit ) ok=true;
			if( h.IsSea() && seaunit ) ok=true;
			if(!ok)
			{
				OrderDone();
				break;
...
void C35::Unit::AutoPilot()
{
	...
	for(i=1;i<=n;++i)
	{
		...
		h.route_cost=0;
		if(!h.Town()) continue;
		if(h.Town()->owner!=owner) continue;
...
{
	UnitType& ut=UnitType::Lookup(type);
	if(!ut.capa.settle) return false;
	Board* brd = owner->game;
	Hexagon& hex = brd->Get(x,y);
	if(hex.Town()) return false;
	if(hex.tile==mount) return false;
	if( hex.Owner() && (hex.Owner()!=owner) ) return false;
	for(Dir6 d=d6_beg;d!=d6_end;++d)
	{
		Hexagon* n = hex.neighbours[d];
		if(n && n->Town()) return false;
	}
	return true;
}
...
bool C35::Unit::CanGo(Dir6 d) const
{
	...
	Hexagon& h1 = owner->game->Get(x,y);
	Hexagon* h2 = h1.neighbours[d];

	if(!h2) return false;


	if( ut.kind == UnitType::Air ) return false;
	...
	if( ut.kind == UnitType::Sea )
	{
		...
		if( h2->units.empty() ) return true;
		return 
			h2->units[0]->owner == owner;
	} else {
		if( ! h2->IsLand() )
		{
			if(h2->units.empty()) return false;
			if(h2->units[0]->owner != owner ) return false;
			for( Unit* u : h2->units )
			{
				if( u->CanLoad(this) ) return true;
			}
			return false;
		} else {
			if( (!h2->IsRealLand()) && (!h1.IsRealLand()) ) return false;
			if(h2->units.empty()) return true;
			return
				h2->units[0]->owner == owner ;
		}
...
void C35::Unit::DoGo(Dir6 d)
{
	...
	if(!h) { OrderDone(); return; }

	if(!h->units.empty())
	{
		if(h->units[0]->owner!=owner) { OrderDone(); return; }
	}

	UnitType& ut = UnitType::Lookup(type);
	if(ut.kind==UnitType::Air) { OrderDone(); return; }
	if(ut.kind==UnitType::Sea)
	{
		if(!h->IsSea()) { OrderDone(); return; }
		if(h->Town()&&(h->Town()->owner!=owner)) { OrderDone(); return; }
		...
		if(!h->IsLand())
		{
			if(h->units.empty()) { OrderDone(); return; }
			for(Unit* u : h->units)
			{
				if(u->CanLoad(this))
				{
...
bool C35::Unit::CanLoad(const Unit* u) const
{
	...
	if( (loaded.size() + 1) > ut_me.load_capa ) return false;
...
bool C35::Unit::CanUnload(int tx,int ty) const
{
	...
	if(hex.units.empty()) return true;
...
void C35::UnitType::LoadBmp(string name)
{
	...
	have_gfx = have_cis;
	SDL_FreeSurface(img);SDL_FreeSurface(tm);
}
...
{
	if(have_gfx==have_ad)   ad.Instance(p->color);
	if(have_gfx==have_ac)   ac.Instance(p->color);
	if(have_gfx==have_cis) cis.Instance(p->color);
}
...
C35::AnimReflection C35::UnitType::GetRefl(string name,short dir,Player* p)
{
	if(have_gfx==have_name)
	{
		...
		/**/ if(ext=="bmp") LoadBmp (nam);
		else if(ext=="ac")  LoadAC  (nam);
		else if(ext=="ad")  LoadAD  (nam);
		if(have_gfx!=have_name)
		{
